...
public class BaseVFXGUI : ShaderGUI
{
    ...
    // 可折叠组名称
    private bool m_StencilOptions = false;
    ...
    // 保证GUI关闭的时候也在走逻辑
    public virtual void OnClosed(Material material, MaterialProperty[] properties)
    {
        if (materialEditor == null) return;
        this.materialProperty = properties;
        this.targetMat = material; // 当前材质球
    ...
    void show()
    {
        ...
        MaterialProperty _DissolveOSUV = FindProperty("_DissolveOSUV", materialProperty);
        MaterialProperty _DissolveOSUVSmooth1 = FindProperty("_DissolveOSUVSmooth1", materialProperty);
        MaterialProperty _SoftValue = FindProperty("_SoftValue", materialProperty);
        MaterialProperty _DistortionSpeedX = FindProperty("_DistortionSpeedX", materialProperty);
        MaterialProperty _DistortionSpeedY = FindProperty("_DistortionSpeedY", materialProperty);
        ...
        MaterialProperty _DissolveIntensity100 = FindProperty("_DissolveIntensity100", materialProperty);
        ...
        MaterialProperty _MaskTexCustomDataON = FindProperty("_MaskTexCustomDataON", materialProperty);
        MaterialProperty _CloseVertexColor = FindProperty("_CloseVertexColor", materialProperty);
        MaterialProperty _UseMaskR = FindProperty("_UseMaskR", materialProperty);
        MaterialProperty _UseDissolveA = FindProperty("_UseDissolveA", materialProperty);
        // 一些基础的
        ...
        GUILayout.Label("小n的特效shader");
        materialEditor.ShaderProperty(_CloseVertexColor, "关闭顶点色(不懂别点)");
        #region 通用设置
        // 折叠组
        ...
        if (m_BaseAttribute)
        {
            ...
            if (targetMat.GetFloat("_VertexAnimTint") < 0.5)
            {
                ...
                if (_BaseMap != null)   // 修改BaseMap的UV方向 start
                {
                    ...
                    // Precompute rotation matrix (cos, -sin, sin, cos) to avoid runtime trig in shader
                    float angle = 0f;
                    var uvDir = (TexUVDir)targetMat.GetFloat("_UseBaseMapUVDir");
                    if (uvDir == TexUVDir.rot90)
                        angle = Mathf.PI / 2f;
                    else if (uvDir == TexUVDir.rot180)
                        angle = Mathf.PI;
                    else if (uvDir == TexUVDir.rot270)
                        angle = Mathf.PI / 2f * 3f;
                    float cosA = Mathf.Cos(angle);
                    float sinA = Mathf.Sin(angle);
                    targetMat.SetVector("_UVRotMatrix", new Vector4(cosA, -sinA, sinA, cosA));
                    #endregion
                }                       // 修改BaseMap的UV方向 end
                materialEditor.ShaderProperty(_BaseMap, "特效贴图");
                EditorGUI.indentLevel++;
                materialEditor.ShaderProperty(_BaseColor, "特效贴图颜色");
                // CustomData1.zw start
                EditorGUI.BeginChangeCheck();
                EditorGUI.showMixedValue = _MainTexCustomDataON.hasMixedValue;
                var MainTexCustomDataON = EditorGUILayout.Toggle("启用customData1.zw 控制MainTex流动一次", _MainTexCustomDataON.floatValue == 1);
                if (EditorGUI.EndChangeCheck())
            ...
            #region mask
            // mask只在透明情况下使用，shader里只涉及a通道
            if ((SurfaceType)targetMat.GetFloat("_Surface") == SurfaceType.Transparent)
            {
                GUILayout.Space(20);
                GUILayout.Label("----------MaskTex 部分----------");
                EditorGUILayout.BeginVertical(EditorStyles.helpBox);
                EditorGUI.indentLevel++;
                ...
                var MaskTexCustomDataON = EditorGUILayout.Toggle("启用customData2.zw 控制MaskTex流动一次", _MaskTexCustomDataON.floatValue == 1);
                if (EditorGUI.EndChangeCheck())
                    _MaskTexCustomDataON.floatValue = MaskTexCustomDataON ? 1 : 0;
                if (_MaskTexCustomDataON.floatValue == 1)
                {
                    targetMat.SetFloat("_MaskTexCustomDataON", 1);
                }
                else
                {
                    targetMat.SetFloat("_MaskTexCustomDataON", 0);
                }
                EditorGUILayout.EndVertical();
                EditorGUI.indentLevel--;
                // CustomData2.zw end
        ...
        if (m_Advanced)
        {
            ...
            if (_Distortion.floatValue == 1)
            {
                ...
                if (_DistortionOpaque.floatValue == 1 || _DistortionTransparents.floatValue == 1)
                {
                    EditorGUILayout.HelpBox("---记得挂上 TransparentWarping.cs ---", MessageType.Warning);
                }
            ...
            // 目前只有溶解有互斥，所以直接改名为溶解了，而不是feature

            if ((SurfaceType)targetMat.GetFloat("_Surface") == SurfaceType.Opaque)
            {

                #region 不透明互斥的功能
                DoPopup(new GUIContent("溶解"), _Feature_Qpaque, Enum.GetNames(typeof(FeatureO)));
                ...
                else if ((FeatureO)targetMat.GetFloat("_Feature_Qpaque") == FeatureO.HardDissolve)
                {
                    ...
                    /// "物体空间采样贴图 开关
                    DrawPropertyToggle(_DissolveOSUV, "物体空间采样贴图");
                    EditorGUI.indentLevel++;
                    if (_DissolveOSUV.floatValue > 0.5f)
                    {
                        // 感觉这个参数会误导美术，关掉了
                        // materialEditor.ShaderProperty(_DissolveOSUVSmooth1, "物体空间采样混合强度");
                        materialEditor.ShaderProperty(_DissolveIntensity100, "溶解强度100");
                    }
                    else
                    {
                    ...
                    // 这部分功能是开启customData1.x就关闭溶解强度面板，但其实这个是可以共存的，故注释掉
                    //EditorGUI.BeginChangeCheck();
                    //if (EditorGUI.EndChangeCheck())
                    //    _DissolveCustomData.floatValue = DissolveCustomDataOn ? 1 : 0;
            ...
            else if ((SurfaceType)targetMat.GetFloat("_Surface") == SurfaceType.Transparent)
            {
                ...
                #region 透明互斥的功能
                DoPopup(new GUIContent("溶解"), _Feature_Transparent, Enum.GetNames(typeof(FeatureT)));
    ...
    // 某些功能为该_SurfaceType独有的，故在切换的时候关闭无效变体
    private void CheckKeyword()
    {
        ...
        {
            // 不透明模式强制关闭
            targetMat.DisableKeyword("_SOFTPARTICLES_ON");  // off SoftParaticles Keyword
        }
