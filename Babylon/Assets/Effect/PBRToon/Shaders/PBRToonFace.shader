    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 PBRToonFaceFrag(Varyings i) : SV_Target0
            {
                ...
                float3 _dbg_rampTex = 1;   // 明暗交界线（faceMapShadow，SDF 脸部阴影）
...

                {
                    ...
                    _dbg_rampTex = faceMapShadow.xxx;
                ...
                #ifdef _DEBUG_SHADOW
                {
                    ...
                    if (debugMode == 1)
                        return float4(_dbg_rampTex, 1);   // Ramp: 明暗交界线（faceMapShadow）
                    else
