...
namespace TATools.FolderCommentTool
{
    ...
    public class FolderCommentManager
    {
        ...
        /// <summary>
        /// 获取全部注释（只读遍历用，可能为 null）
        /// </summary>
        public System.Collections.Generic.List<FolderCommentData> GetAllComments()
        {
            return _database?.Comments;
        }

        /// <summary>
        /// 按快捷定位槽位（1-9）查找注释，找不到返回 null
        /// </summary>
        public FolderCommentData GetCommentBySlot(int slot)
        {
            if (_database == null || slot <= 0)
                return null;

            var list = _database.Comments;
            if (list == null)
                return null;

            foreach (var c in list)
                if (c != null && c.shortcutSlot == slot)
                    return c;
            return null;
        }

        /// <summary>
        /// 设置某资产/文件夹的快捷定位槽位（0 = 清除）。
        /// 仅对“已有注释”的 GUID 生效；并保证槽位全局唯一（占用同槽位的旧注释会被清 0）。
        /// </summary>
        public void SetShortcutSlot(string guid, int slot)
        {
            if (_database == null || string.IsNullOrEmpty(guid))
                return;

            var data = _database.GetComment(guid);
            if (data == null)
                return; // 没注释就不分配槽位（槽位只服务于带注释的资产）
            ...
            if (slot > 0)
            {
                ...
                if (list != null)
                    foreach (var c in list)
                        if (c != null && c != data && c.shortcutSlot == slot)
                            c.shortcutSlot = 0;
            }

            data.shortcutSlot = slot;
            SaveDatabase();
