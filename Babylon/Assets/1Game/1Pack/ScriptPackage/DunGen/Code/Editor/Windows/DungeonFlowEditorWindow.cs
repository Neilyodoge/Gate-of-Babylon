	...
	public sealed class DungeonFlowEditorWindow : EditorWindow
	{
		...
		private void HandleNodeContextMenu(GraphNode node)
		{
			...
			if (node.NodeType == NodeType.Normal)
				menu.AddItem(new GUIContent("删除 " + (string.IsNullOrEmpty(node.Label) ? "节点" : node.Label)), false, NodeContextMenuCallback, GraphContextCommand.Delete);
		...
		private void HandleLineContextMenu(GraphLine line)
		{
			...
			menu.AddItem(new GUIContent("在此添加节点"), false, LineContextMenuCallback, GraphContextCommand.AddNode);
			menu.AddItem(new GUIContent("拆分区段"), false, LineContextMenuCallback, GraphContextCommand.SplitLine);

			if (Flow.Lines.Count > 1)
				menu.AddItem(new GUIContent("删除区段"), false, LineContextMenuCallback, GraphContextCommand.Delete);
		...
		{
			var window = EditorWindow.GetWindow<DungeonFlowEditorWindow>(false, "地牢流程", true);
			window.Flow = flow;
		}
