...
namespace DunGen.Editor
{
	...
	public class LocalPropSetInspector : UnityEditor.Editor
	{
		...
		private SerializedProperty propWeights;
		...
		private void OnEnable()
		{
			...
			propWeights = serializedObject.FindProperty("PropWeights");
		}
		...
		public override void OnInspectorGUI()
		{
			...
			EditorGUILayout.PropertyField(propWeights, Label.Props);
			...
			if (GUILayout.Button("Add Selected Props"))
			{
				...
				{
					if (!propSet.PropWeights.ContainsValue(go))
						propSet.PropWeights.Entries.Add(new WeightedEntry<GameObject>(go));
				}
				...
				if (dragTargetRect.Contains(evt.mousePosition) && validGameObjects.Any())
				{
					...
					if (evt.type == EventType.DragPerform)
					{
						...
						foreach (var dragObject in validGameObjects)
							propSet.PropWeights.Entries.Add(new WeightedEntry<GameObject>(dragObject));
