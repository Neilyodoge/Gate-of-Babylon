...
> - **V0.4.1（关卡 A 试接）Edgar PRO Grid3D**：默认 Provider 为 `EdgarMapProvider`，`EdgarDungeonRuntime` 一次生成 12 个白膜主体房及走廊，支持随机双向出生、对侧单 Boss、步行进房、门锁、清场解锁与回访状态。silverua/StsMap 暂留作删除前回退资产。详见 §10.4。
...
| `RoomContentResolver` | 按 Role/District/深度/固定 Seed 从 `关卡数据库.asset` 解析内容 |
...
当前关卡 A 每境由 Edgar 一次生成 12 个常驻主体房，分为 Outer / Transition / Inner；每局随机选择一侧出生，只激活对侧单 Boss。
...
- **高度雾（2026-08-05）**：URPFog `urp14` 以嵌入式 UPM 包放在 `Packages/moe.meryuhi.effects.fog/`；`N_RenderData.asset` 挂一份 `Full Screen Fog Renderer Feature`，参数由 `N_VolumeProfile.asset` 的 `Meryuhi/Full Screen Fog` Override 控制。当前 25m 俯视相机基线为 `Height / Intensity=0.04 / Exponential / Density=0.015`；Graphics 默认与 PC High 画质均使用 `N_RenderAsset`，避免 Editor/Player 落到不同 Renderer。
...
| `LevelDesign/Edgar/EdgarMapProvider.cs` | **当前默认实现** | 提供 12 房布局；随机选择 `O0/I4` 出生端，只标记对侧单 Boss，并固定商店、双事件与双精英节点。导航 UI 返回 false。 |
| `LevelDesign/Edgar/EdgarDungeonRuntime.cs` | Grid3D 运行桥 | 加载 `WhiteboxLevelGraph`，按任意降落节点排序；建立布局局部 +X 后按 Seed 整体旋转，抽取 Prefab `PlayerSpawn`/Bounds，并创建触发器及战斗门锁。 |
| `Core/Level/RoomFactory.cs` | `IRoomFactory`+`RoomSpawnContext` | 按 `RoomType` 创建房间；`Landing` 使用无遭遇、无锁门的独立分支，上下文同时携带实体 Prefab 的 District。 |
| `Core/Level/RoomRuntimeController.cs` | 房间状态机 | 保存 RoomContent ID、Encounter Seed、门锁/清场状态；驱动 `IRoomContentHandler`，回访不重抽。 |
| `Core/Level/CombatRoomContentHandler.cs` | 战斗 Handler | 把 Battle/Boss 接到 Encounter；普通小怪消费自动组队方案，Boss消费独立条件池，并复用 `BattleRoom` 的奖励和清场事件。 |
...
- **游戏侧资源**：`Resources/LevelDesign/EdgarGrid3D/WhiteboxLevelGraph` 是默认 12 节点图；`Resources/LevelDesign/EdgarGrid3D/RoomTemplates` 是唯一模板基础库，`Room_01～03 / Corridor_01～02` 为源模板，12 个节点专用主体 Prefab 与 4 个连接白膜生成到同目录 `Generated/`。旧 `Prefabs/LevelDesign/EdgarGrid3D`、`Prefabs/LevelDesign/Templates` 与废弃 `PrototypeLevelGraph` 已删除。
- **白膜构建器**：`Editor/EdgarWhiteboxBuilder.cs` 提供「仙途秘境/关卡工具/生成 Edgar 白膜关卡」。它兼容 `Blocks / Objects` 两种 Edgar 轮廓根节点，复制基础模板并保留原始空间几何，按节点连接度裁剪 Door Handler，再写入语义标记、分区/标签/有效范围/三类刷怪范围并重建图。12 节点共 22 个门端严格对应 11 条图边；自动 Corridor 池仅使用直廊与转角。
- **`EdgarDungeonRuntime`**：运行时显式构造 `DungeonGeneratorGrid3D`；生成根统一缩放 5 倍，按任意降落节点重排首房，先建立外环→内环的布局局部 +X，再按 Seed 追加 `0 / 90 / 180 / 270°` 根节点整体旋转。实际落点从房内多个 `PlayerSpawn` 稳定抽取；随后遍历生成态 `RoomInstanceGrid3D.Doors`，用角色胶囊半径校验门槛外 1.5 米通行体积。
- **`EdgarMapProvider`**：`StartRun/OnEnterRealm` 先确定外环或内环出生区，固定使用离商店最远的 `O4` 或 `I3` 作为降落房并激活对侧 Boss，到正式请求首房时才生成几何。`GetFloors` 返回普通 6、精英 2、事件 2、商店 1、Boss 1；`RequiredBossCount=1`。
...
- **内容解析**：`RoomSpawnContext` 从实体 Prefab 携带 `DungeonRoomAuthoring.District`，`RoomContentResolver` 优先使用该值；非 Edgar 路径才回退按索引估算。`RoomRuntimeState` 保存内容 ID、固定 Encounter Seed、选定 Boss ID 与材料点是否已经生成。
...
- **Boss 条件池**：按 District、PrefabTags、RequiredFlags/BlockedFlags 和权重筛选；固定房间 Seed 抽取后写入 `RoomRuntimeState.SelectedBossID`，Boss 形态由同一 `关卡数据库.asset` 解析。
...
- **当前边界**：12 房区域白膜、Prefab 分区标签、远距安全降落、整图旋转、材料采集、三类 Story 运行闭环和单 Boss 代码已接；正式降落美术、模板数据资产化、探索地图及局部地标关系的 Playtest 仍未完成。非 Boss 撤离本轮暂缓。
