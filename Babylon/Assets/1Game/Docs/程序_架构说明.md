...
> - **ProjectR P3.52角色原型模型已接入**：`ArtRes/Package/Character/palu`保存三宠与Ronin的Generic骨架FBX、URP材质和视觉Prefab；序章三选一只替换表现子物体，默认Boss通过`MonsterPrefabs`切换视觉Prefab，既有选择与战斗组件不变。四套模型当前均无AnimationClip。
