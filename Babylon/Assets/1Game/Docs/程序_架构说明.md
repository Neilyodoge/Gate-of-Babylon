...
- **`EdgarMapProvider`**：`StartRun/OnEnterRealm` 先确定外环或内环出生区，再从 `O2 / O4` 或 `I0 / I3` 抽取降落房并激活对侧 Boss，到正式请求首房时才生成几何。`GetFloors` 返回普通 6、精英 2、事件 2、商店 1、Boss 1；`RequiredBossCount=1`。
...
- **内容解析**：`RoomSpawnContext` 从实体 Prefab 携带 `DungeonRoomAuthoring.District`，`RoomContentResolver` 优先使用该值；非 Edgar 路径才回退按索引估算。`RoomRuntimeState` 保存内容 ID、固定 Encounter Seed 与选定 Boss ID。
...
- **当前边界**：12 房区域白膜、Prefab 分区标签、区域内随机安全降落、整图旋转和单 Boss 代码已接；降落房视觉提示、Story Template、捷径、探索地图及局部地标关系的 Playtest 仍未完成。
