...
- **`EdgarDungeonRuntime`**：运行时显式构造 `DungeonGeneratorGrid3D` 配置并同步生成；生成根统一缩放 5 倍，过滤走廊后提取 14 个房间的 Bounds、落点与尺寸，并为每房创建 `EdgarRoomTrigger`。战斗激活时在门位创建 `__CombatGate`，清场后解锁。
...
- **遭遇注入**：`RoomSpawnContext.contentRoot` 把当前 Edgar 房间实例交给 `BattleRoom`；后者读取 `DungeonContentSocket` 的 `EnemySpawn/BossSpawn` 世界坐标。每个房间以自己的敌人集合判断清场，插槽不足时按波复用，Boss 由专用插槽生成；禁止用全场景标签数量驱动房间状态。
...
- **当前边界**：步行穿门和回头探索已通，但尚未接入 Outer/Transition/Inner 区域标签、故事模板、捷径和探索地图；两个 Boss 暂按房间索引分配。
