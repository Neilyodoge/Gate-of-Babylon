...
> - 关卡结构为 **3 层 × 每层 12 关 × 3 路线**（非旧「6 层」）；已有 TreeMap 程序化地图、多槽存档、Build 局外背包、大秘境（Rift）。
...
> - **V0.4.1（进度重构线）地图接入 silverua 杀戮尖塔原生全图**：默认 provider 换为 `SilveruaMapProvider`；silverua 原生生成+渲染（`MapManager`/`MapViewUI`/`MapPlayerTracker`）经 `StsMapScreen` 运行时装配（Canvas+双 ScrollRect），地图=进度真源、每局重生成、DOTween 动效；旧 `TreeMapGenerator`/`TreeMapUI`/`LevelDesignMapProvider` **已物理删除**，`LevelDesignDirector` 只留 Flag/状态重置 + Boss 形态解析。详见 §10.4。
> - **V0.4.1 §11.4.6 BUG/调整（9 项）**：①局外「挂空」——`GameManager.EnterVillageHub` 进村 `ClearPlayerLoadout()` 清 Q/E/R+增强链（普攻保留）；②**世界掉落总开关** `GameManager.EnableWorldDrops=false`——所有 `*.Spawn`（技能/素材/模块/宝箱/遗产）early-return，仅碎片货币保留；③每境房间数 `DefaultMapConfig.layers` 12→24；④**统一 3 关**——换境首间也走 STS 全图（`EnterNextRoomWithChoice`），`GameHUD` 显示「第 X/N 关」；⑤`BuildSnapshot.ApplyToPlayer` 装备后广播 `SkillEquipped`+刷新 `SkillBarUI`；⑥新增 `RiftTierSelectUI`（大秘境进入前选 1~100 层）；⑦`VillageHub` 移除配置使/Build 管理使（BD 只在大秘境缓冲区）；⑧`Minimap.SetVisible`——局外/大秘境隐藏；⑨`PauseMenu` 加「存档」按钮 → `SaveSystem.Save()`。
...
4. 如需出现在地图上：在 `SilveruaMapProvider.ToRoomType()`（`NodeType→RoomType`）补映射，或在 `GameManager.GenerateLevelLayout()` 固定布局中配置
...
| `Core/Level/IMapProvider.cs` | 接口 + `MapProviders.Current`（默认 `SilveruaMapProvider`） | 地图/拓扑抽象：`StartRun` / `GetFloors` / `GetEnemyScale` / `GetRarityBias` / `GetHasStageReturn` / `TryTriggerRoomEvent` / `MarkCurrentCleared` / `CurrentNodeHasNext` / `TryShowNavigation` / `CurrentActId`。 |
| `LevelDesign/StsMap/SilveruaMapProvider.cs` | **当前默认实现** | 桥接 silverua 原生地图：`StsMapScreen` 承载 `MapManager/MapViewUI/MapPlayerTracker`，节点 `NodeEntered` → `NodeType→RoomType`；数值查询走 `ConfigDatabase.MapStructures`；`StartRun/OnEnterRealm` 兼管整局/换境的 Flag/状态重置（调 `Director.StartNewRun/BeginAct`）与按 Act 选 `MapConfig`（`SetActConfig`）。 |
...
```
GameManager ──> IMapProvider (MapProviders.Current) ──> [SilveruaMapProvider] ──> StsMapScreen / silverua Map(Manager/View/PlayerTracker) + ConfigDatabase.MapStructures
...
#### 当前现状（V0.4.1 进度重构线）：接入 silverua 杀戮尖塔**原生全图**（默认 `SilveruaMapProvider`）

在 UI 栈已统一为 uGUI+TMP+DOTween 后（见 §4.6 / V0.4.6），冲突消失，遂**直接接入 silverua [杀戮尖塔地图](https://github.com/silverua/slay-the-spire-map-in-unity) 的原生生成 + 渲染**（不再是 V0.4.5 的「只移植算法、沿用 UITK」），地图=进度真源、每局重生成：

- **资产**：silverua `Scripts/Prefabs/Sprites/Materials/Resources/Scriptable Objects` + 自带 DOTween 落入 `1Game/StsMap/`；预制体 `MapObjectsUI Variant` 移入 `1Game/StsMap/Resources/StsMapObjectsUI`。
- **不持久化**：剔 `Newtonsoft.Json`（`Map/Node/MapManager` 去序列化、`SaveMap()` 置空、`Start()` 无条件生成）；去 `OneLine` 编辑器特性。
- **表现层 `StsMapScreen`**（`Scripts/LevelDesign/StsMap/`）：运行时代码搭 Overlay Canvas + 遮罩 + 标题 + 两个 `ScrollRect`，实例化 `Resources/StsMapObjectsUI` 并给 `MapViewUI` 回填 ScrollRect 引用（新增 `MapViewUI.SetScrollRects`）；负责显隐 / 按境重生成 / `NodeEntered` 回调后自动隐藏。
- **`MapPlayerTracker` 桥接**：`EnterNode` 触发静态事件 `NodeEntered`；进入延时改 `SetUpdate(true)`（非缩放时间，避免 `timeScale=0` 冻结）。
- **路线预算拓扑（§11.4.7）**：`DefaultMapConfig.useRouteBudget=true`；`MapGenerator` 每境随机取 24~26 个**总节点**（含共享 Boss），拆到 3 条长短不同的独立路线并汇入 Boss，不再把 `layers.Count` 误当成每条路线长度；单路线精英≤2、商店≤4。
- **`SilveruaMapProvider : IMapProvider`**：`GetFloors()` 只给 1 个初始 Battle 占位；每清一间由 `StsMapScreen.CurrentNodeHasNext`（当前节点 `outgoing`）驱动 `GameManager` 动态扩展，选到共享 Boss 后自然结束本境。`TryShowNavigation` 始终弹全图（含 Boss 节点），点选后 `NodeType→RoomType`；数值查询仍走 `ConfigDatabase.MapStructures`。
- **GameManager / 准备区**：`Awake` 无条件使用 `SilveruaMapProvider`；进入普通准备区不自动弹技能，触发入口后 `SkillSelectUI` 选完才进入首节点；普通准备区和大秘境缓冲区复用 `PreparationGate` 提供返回基地出口，正式战斗区不生成返回门。
- **模块奖励可靠加载**：`Resources/ModuleCatalog.asset` 显式引用 `Data/Modules` 下全部 `ModuleDef`，`ModulePoolLoader` 优先从 Catalog 加载，避免非 Resources 资产在 Player 中变空；`RewardPickUI` 每局首战模块、之后技能/模块交替；奖励/商店自动装配失败时写入 `ModuleInventory`，不会静默丢失。

> **遗留清理（2026-07-30 已完成）**：V0.4.5 的 `TreeMapGenerator`/`TreeMap`/`TreeNode`（`LevelDesign/Map/`）、`TreeMapUI`、`LevelDesignMapProvider` 已**物理删除**；`LevelDesignDirector` 去掉全部地图拓扑成员，只留 Flag/状态重置 + Boss 形态解析；房间剧情事件统一走 `LevelDesignBootstrap` 线性调度表。
> **`MapConfig` 按 Act 分化**：已留接口 `StsMapScreen.SetActConfig(act)`（从 `MapViewUI.allMapConfigs` 按 act-1 选），当前列表仅 1 个 `DefaultMapConfig`（各境共用），追加对应 `MapConfig` 即分化，无需改代码。
> **`LevelRoomType` 保留**（有意，非双存债务）：它是配表 schema 枚举（`RoomSocketRow.TypeEnum` ↔ `Room_Socket_Group_Config.csv` 的 RoomType 整数列）；silverua 侧只用 `NodeType→RoomType`，已不再触碰它，故不合并。
