...
> 📌 **当前基线（2026-07 快照）**：
> - 技能系统已切换为 **模块化技能**（Trigger/Effect/Modifier/Universal，GDD §4/§6）；旧「灵物系统」默认屏蔽（`FeatureFlags.EnableSpiritItems=false`），下文 §4.4 灵物章节为历史遗留，勿据此假设灵物在游戏内生效。
...
| `InventoryUI` | 灵物背包UI：Tab键开关，显示持有灵物/Synergy/属性总览 |
| `SkillBarUI` | 技能栏UI：Q/E/R槽位显示，CD遮罩，充能层数 |
| `BuffBarUITK` | 状态栏（UITK）：buff/debuff chip + 鼠标悬停 tooltip（v0.6.2）；取代旧 `StatusEffectHUD` |
| `SpiritRootSelectUITK` | 化身选择面板（UITK）：卡片 + roleTag 角色定位标签（v0.6.2） |
| `ExtractResultPanel` | 撤离成果面板（UITK）：灵力/历练/素材明细 + 层深倍率（v0.6.2） |
| `GrowthUITK` | 化身成长树面板（UITK）：系精通节点解锁 + 渐进分支门控（v0.6.3） |
| `BattleRewardUI` | 战后三选一灵物卡牌（UITK）：通关弹窗选奖励（v0.6.3） |
...
> 未做（后续可选）：`IMapView` 表现层接口（把 `TreeMapUI` UITK 藏到接口后，便于替换为 uGUI 版地图）；`LevelRoomType` 与 `RoomType` 目前仍双存、在 provider 边界映射（保留 TreeMap 生成内部用 `LevelRoomType`）。
