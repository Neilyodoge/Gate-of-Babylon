...
> - **V0.4.1（关卡 A 试接）Edgar PRO Grid3D**：默认 Provider 为 `EdgarMapProvider`，`EdgarDungeonRuntime` 一次生成实体房间/走廊并提供步行进房触发、门锁、清场解锁与回访状态；区域双 Boss 均击败后才开放下一层出口。silverua/StsMap 暂留作删除前回退资产。详见 §10.4。
...
当前关卡 A 每境由 Edgar 一次生成 14 个常驻主体房，目标分为 Outer / Transition / Inner，含双 Boss。
...
- **高度雾（2026-08-05）**：URPFog `urp14` 以嵌入式 UPM 包放在 `Packages/moe.meryuhi.effects.fog/`；`N_RenderData.asset` 挂一份 `Full Screen Fog Renderer Feature`，参数由 `N_VolumeProfile.asset` 的 `Meryuhi/Full Screen Fog` Override 控制。初始 `Intensity=0`，默认不改变画面。
...
- **游戏侧资源**：`Resources/LevelDesign/EdgarGrid3D/` 保存独立 `PrototypeLevelGraph` 与 `GeneratorSettings`；图当前引用 `Prefabs/LevelDesign/EdgarGrid3D/FantasticDungeon/` 下直接拼装的 6 个房间、2 个走廊及门连接/封堵 Prefab，不再使用 Edgar Basics 作为运行模板。
- **`EdgarDungeonRuntime`**：运行时显式构造 `DungeonGeneratorGrid3D` 配置并同步生成；生成根统一缩放 5 倍，过滤走廊后提取 14 个房间的 Bounds、落点与尺寸，并为每房创建 `EdgarRoomTrigger`。只提供显式 `SetRoomLocked`，是否锁门由房间内容的 `LockPolicy` 决定。
- **`EdgarMapProvider`**：`StartRun/OnEnterRealm` 只重置状态和布局，不立即生成几何；`GameManager` 正式请求首房落点时才生成新 Seed 地牢，避免与村庄/准备房重叠。`GetFloors` 当前返回 14 间（普通 Battle + 末房 Boss），数值仍走 `ConfigDatabase.MapStructures`；`TryShowNavigation=false`。
- **`GameManager`**：首房获取 Edgar 落点后进入；后续由 `EdgarRoomTrigger` 在玩家步行进入未清房时调用 `EnterEdgarRoom` 激活遭遇，不再逐房传送。已清索引存入当层集合，回访不重复生成。普通清场不创建传送门；两个 Boss 房均清理后才调用 `SpawnLevelCompletePortal`。Edgar 生成/落点失败会直接抛错，不再回退旧 `RoomBuilder` 几何。
- **内容解析**：`关卡数据库.asset` 用 Role/District/ActivationMode/LockPolicy 描述房间；`RoomRuntimeState` 保存内容 ID、固定 Encounter Seed 与选定 Boss ID。当前 District 由稳定物理房间索引暂算，后续区域节点图可替换键来源。
...
- **Prefab 刷新范围**：`DungeonEnemySpawnArea` 支持 Box/Sphere/Capsule/Mesh Collider。`BattleRoom` 优先按分类、容量、玩家距离和点间距采样；无范围的旧模板继续使用 `EnemySpawn/BossSpawn` 离散点。
...
- **当前边界**：战斗 Handler 已接；商店、事件、休息、军械库仅保留 `IRoomContentHandler` 与 `ContentConfigID` 扩展点，尚未迁移具体业务。正式区域节点图、Prefab 分区标签、故事模板、捷径和探索地图仍未接，两个 Boss 暂按房间索引分配。
