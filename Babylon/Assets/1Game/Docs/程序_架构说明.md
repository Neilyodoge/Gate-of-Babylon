...
- **`SilveruaMapProvider : IMapProvider`**：每境一张分叉图；`GetFloors()` 用「层数=房间数、末间 Boss」脚手架（层数取 `MapConfig.layers.Count`，境数取 `GameManager.RealmCount`）；`TryShowNavigation` 弹全图、点节点后 `NodeType→RoomType`（Minor→Battle / Elite→Elite / RestSite→Rest / Treasure→Treasure / Store→Shop / Boss→Boss / Mystery→Event），Boss 为唯一收束点不弹图；数值查询仍走 `ConfigDatabase.MapStructures`。
- **GameManager**：`Awake` 无条件 `MapProviders.Current = new SilveruaMapProvider()`；首房也经地图点选（`OnPrepRoomComplete → EnterNextRoomWithChoice`）；遗产模块改由首个 `SpawnCurrentRoom` 注入。
