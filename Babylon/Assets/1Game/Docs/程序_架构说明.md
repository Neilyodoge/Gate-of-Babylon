...
> - 关卡为 silverua STS 分叉地图；跨局进度使用 **3 个独立 JSON 槽位 + schema v3 永久技能/模块解锁**。局内 Build 不持久化，大秘境（Rift）与 Build 背包已删除。
...
- **GameManager / 准备区**：`Awake` 无条件使用 `SilveruaMapProvider`；进入普通准备区不自动弹技能，触发入口后 `SkillSelectUI` 选完才进入首节点；普通准备区保留 `PreparationGate` 返回基地出口，正式战斗区不生成返回门。
