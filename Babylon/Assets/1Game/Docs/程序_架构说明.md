...
> - **ProjectR P3.51序章V5正式场景已集成**：`StarterPrologue`改以HIGHLANDS环境、Terrain与场景光照为底，运行时根和玩法对象从旧场景迁入；玩家、角色、触发器、敌人和转场位置以关闭的V5 Guide逐项对齐，旧V3环境／几何碰撞停用并保留场景备份。
> - **ProjectR P3.52角色原型模型已接入**：`ArtRes/Package/Character/palu`保存三宠与Ronin的骨架FBX、URP材质和视觉Prefab；序章三选一只替换表现子物体，默认Boss通过`MonsterPrefabs`切换视觉Prefab，既有选择与战斗组件不变。源模型均无AnimationClip；P3.58已将Ronin改为Humanoid供序章宿主重定向KayKit动作。
> - **ProjectR P3.54序章导航已改为预烘**：`03_序章玩法布局_V5`上常驻一个按世界体积收集、只取PhysicsColliders的`NavMeshSurface`，数据存为`Scenes/StarterPrologue/StarterPrologue_NavMesh.asset`随场景加载；`StarterPrologueSceneBootstrap.BuildNavigation`先查`NavMesh.CalculateTriangulation`，有数据就直接返回，只有缺失时才调用`DungeonNavMeshRuntime.BuildFor(owner, clearing)`兜底。`DungeonNavMeshRuntime`为此新增`Bounds?`重载，传null时维持Edgar地牢原有的子物体收集。手工场景不要再用运行时烘焙：HIGHLANDS环境网格未开放读取，运行时构建在编辑器可用但打包后会丢失障碍。
> - **ProjectR P3.57序章敌宠使用局部Prefab覆盖**：`EnemyBase.Spawn(position, hp, damage)`旧签名保留并转发到新增的四参重载；只有`StarterPrologueTrainingEncounter`向第四参传入序列化的`AgitatedMonkey.prefab`，因此全局`MonsterPrefabs`及普通关卡、精英、Boss、召唤物均不受影响。Monkey使用Humanoid Avatar和独立Animator Controller，`KayKitLocomotionDriver`按实际位移驱动Speed，并在`EnemyBase.StartAttackPrep`通过参数存在性检查触发Attack；不含Attack参数的旧KayKit Controller保持原行为。
> - **ProjectR P3.58失控宿主使用独立Ronin表现覆盖**：`StarterProloguePossessedHost.Spawn`新增可选视觉Prefab参数，`StarterPrologueTrainingEncounter`只为第二阶段传入`PossessedHost_Ronin.prefab`；缺失时保留原胶囊和程序能量武器回退。Ronin专用Prefab嵌套共享Boss视觉，但使用独立Humanoid Controller和`KayKitLocomotionDriver`；宿主三种动作触发单手斜斩，运行时只实例化武器材质并驱动Emission，不会改写共享材质。Monkey的两套URP材质现绑定程序纹理，仍按内部验证资产管理。
> - **ProjectR P3.59序章Monkey禁用动态遮挡并持续更新蒙皮**：HIGHLANDS预烘遮挡数据会在特定位置误剔除动态SkinnedMesh的Forward Pass，但阴影通道仍绘制，表现为Scene可见而Game只有影子；`AgitatedMonkey.prefab`因此关闭`allowOcclusionWhenDynamic`，并启用`updateWhenOffscreen`与Animator `AlwaysAnimate`避免移动／攻击中蒙皮变换冻结，不影响相机对静态环境的全局遮挡剔除。`StarterPrologueTrainingEncounter`另把两只实例的CharacterController和NavMeshAgent半径设为0.6米，并分配35／65避障优先级，防止共用玩家目的地时视觉重叠；该参数不进入`EnemyBase`。Monkey材质关闭接收阴影并使用低强度Emission，同时仍保留投射阴影。
> - **ProjectR P3.60动态角色统一安全渲染默认值**：`DynamicCharacterRendering.Apply`集中把Animator设为`AlwaysAnimate`，并对SkinnedMesh关闭`allowOcclusionWhenDynamic`、启用`updateWhenOffscreen`。`PlayerAnimator`覆盖玩家初始化和运行时换模，`MonsterPrefabs.InstantiateMonster`覆盖普通怪物生成，`KayKitLocomotionDriver`覆盖不经过统一怪物工厂的KayKit角色。该策略有明确的离屏动画／蒙皮性能成本，当前作为角色显示稳定优先的临时全局默认，正式性能验收后再分级收窄。
> - **ProjectR P3.63序章事故揭示与刃铠灵收尾已接通**：`StarterSpiritChoiceWorldEntity`在运行时隐藏三宠场景Renderer并生成无拾取逻辑的球形容器，`StarterPrologueChoiceTrigger`按“照料员→容器→刃铠灵”聚焦后复用三卡选择；`StarterPrologueCaretakerFear`提供照料员惊慌占位。Ronin原型的玩家可见语义、目标和Boss条改为刃铠灵／凶势，凶势归零后手工位移撤离而非普通死亡或人宠分离。战斗只提交`RescueCompleted`，`StarterPrologueHomeCompletion`由Bootstrap挂到固定`HomeReturn`标记，玩家进入后才加载家园；场景重进不再自动转场。
...
- `UI/StarterSpiritChoiceWorldEntity.cs`：正式灵宠场景Prefab的种族身份、点击／悬停事件与聚焦反馈适配器；序章选择前会隐藏灵宠Renderer并生成球形容器占位，确认后只短暂显现所选灵宠。
...
- `Core/StarterPrologueMarker.cs`：序章场景的玩家出生、救援区、战斗入口和家园转场稳定标记。
- `Core/StarterPrologueChoiceTrigger.cs`：容器共同接近范围触发“照料员→容器→远端刃铠灵”揭示与初契UI，并在选择后恢复单宠附着。
- `Core/StarterPrologueCaretakerFear.cs`：照料员在事故揭示和战斗期间的蜷身颤抖占位表现，战后退场前恢复基础姿态。
- `Core/StarterPrologueAttachmentGate.cs`：首次附着检查点完成前保持训练场入口碰撞。
- `Core/StarterPrologueEnemySpawnMarker.cs`：白盒中的近战／远程稳定出生角色标记。
- `Core/StarterPrologueTrainingEncounter.cs`：2只躁动Monkey→1只刃铠灵的两阶段生成、归属击杀计数、失败场景恢复、`RescueCompleted`存档与固定回家点指引编排。
- `Core/StarterPrologueTrainingDummy.cs`：无攻击／无掉落的机制展示靶，保留IDamageable、结构化伤害和灵宠显化入口。
- `Core/StarterPrologueCompletionGate.cs`：救援完成后关闭回家路径阻挡。
- `Core/StarterPrologueHomeCompletion.cs`：固定`HomeReturn`标记上的玩家触发器；只在`RescueCompleted`时请求加载独立家园，不在序章场景提交`Completed`。
- `Prefabs/Tutorial/StarterPrologueWhitebox.prefab`：旧四段白盒的玩法组件合同源，不再作为正式序章Scene的视觉布局依赖。
- `Prefabs/Tutorial/StarterPrologueLayoutV5_BlockoutGuide.prefab`：正式序章中关闭的空间与关键节点对齐基准，不承载运行时玩法或碰撞。
- `Scenes/StarterPrologue.unity`：以HIGHLANDS环境、Terrain、Lighting、Volume和反射探针为底，包含Runtime根与按V5 Guide对齐玩法对象的独立新手场景；旧场景备份位于`Scenes/Backups/`。
...
- `Editor/ProjectR/Tests/ProjectRP0ContractTests.cs`：155项载体、灵宠、回路、首批三宠、单宠三载体、UI主题资源、附着UI映射、首次强制附着、初契展示／确认、序章恢复、白盒Prefab／V5 Scene依赖、验证战状态、真实编队／命中／弹射、来源诊断、结构化结果、Legacy结算旁路及存档测试，可从「ProjectR/开发工具/运行载体与回路契约测试」执行。
...
- 事件类型较多，新增ProjectR事件时按领域分组，不继续扩大旧入口。
- `IDamageable.OnDamage`信息不足，新增来源归因走结构化战斗结果旁路。
- 部分敌人投射物／VFX未走对象池，迁移时按ProjectR对象生命周期适配。
