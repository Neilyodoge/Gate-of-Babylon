...
> - **V0.4.5 地图统一为单一 STS 分叉全图**：`TreeMapGenerator` 移植 silverua 杀戮尖塔式生成（深度=每境房间数、宽度分叉，零新依赖，沿用 UITK `TreeMapUI`）；导航统一走全图、每境按 ActID 惰性重生成并与房间推进锁步；已**删除** `RoomChoiceUI` 三选一。详见 §10.4。
...
| `PostProcessSetup` | `Core/PostProcessSetup.cs` | 后处理效果管理 |
...
| `PostProcessSetup` | 后处理 | `UpdateAtmosphere(level, total)` `TriggerDamagePulse()` |
| `TopDownCamera` | 俯视角相机 | 平滑跟随玩家 |
| `Demo1Setup` | 场景自动搭建 | 运行时动态创建所有GameObject、UI、Animator |
...
| `BuffBarUITK` | 状态栏（UITK）：buff/debuff chip + 鼠标悬停 tooltip（v0.6.2）；取代旧 `StatusEffectHUD` |
| `ExtractResultPanel` | 撤离成果面板（UITK）：经验/历练/素材明细 + 层深倍率（v0.6.2） |
| `RewardPickUI` | 战后 / 商店三选一（技能 或 模块）UITK 面板（V0.4.1） |
...
| `PlayerDamaged` | 玩家受伤 | GameHUD、PostProcessSetup |
...
- **运行时创建**：所有GameObject均由代码动态创建（`Demo1Setup.cs`），不依赖预制体
...
4. 如需出现在地图上：在 `LevelDesign.LevelRoomType` + `LevelDesignMapProvider.Map()` 补映射，或在 `GameManager.GenerateLevelLayout()` 固定布局中配置
...
| `Core/Level/IMapProvider.cs` | 接口 + `MapProviders.Current` | 地图/拓扑抽象：`StartRun` / `GetFloors` / `GetEnemyScale` / `GetRarityBias` / `GetHasStageReturn` / `TryTriggerRoomEvent` / `MarkCurrentCleared` / `CurrentNodeHasNext` / `TryShowNavigation` / `CurrentActId`。 |
| `Core/Level/LevelDesignMapProvider.cs` | 默认实现 | 基于第 12 章 LevelDesign（`LevelDesignDirector`+`ConfigDatabase`+`TreeMap`）实现上述接口，并在**边界**完成 `LevelRoomType → RoomType` 映射。 |
...
```
GameManager ──> IMapProvider (MapProviders.Current) ──> [LevelDesignMapProvider] ──> LevelDesign/ConfigDatabase/TreeMap
...
#### V0.4.5 现状：单一 STS 分叉全图（默认 `LevelDesignMapProvider`）

评估 silverua [杀戮尖塔地图](https://github.com/silverua/slay-the-spire-map-in-unity) 后（其全套是 uGUI+2D 精灵+DOTween+OneLine+Newtonsoft 栈，与本项目 UITK/3D URP 技术栈不符），采用 **option A：只移植生成算法、沿用项目 UITK 表现**：

- **生成**：`TreeMapGenerator.Generate` 移植 silverua 思路——固定列宽 grid + 多条起点→Boss 的列随机游走路径 + 分叉汇合，产出现有 `TreeMap/TreeNode`。深度 = 每境房间数（`Map_Structure_Config.MaxNodes`），宽度固定 4 列。零新依赖。
- **导航（单一全图）**：`LevelDesignMapProvider.TryShowNavigation` 每境按 `ActID = GameManager.CurrentLevel + 1` 惰性 `BeginAct` 重生成分叉图，使分叉图 `CurrentNode` 推进与 `_currentRoomInLevel` 锁步；Boss 房不弹图（线性叙事）。
- **脚手架分工**：`GetFloors()` 返回 `null` —— 分叉图**只**做导航/可视化/房型覆盖，不再当线性房间脚手架（否则 12 层深会被误当 12 个境）；线性 12 间脚手架回到 `GameManager.fixedLayout`，实际房型由玩家在全图上的选择经 `OverrideNextRoomType` 覆盖。
- **已删除**：`RoomChoiceUI`（三选一）及其 uxml/uss，连同 `GameManager.TryShowRoomChoice/BuildRoomCandidates`。这修复了旧「首进全图、再进三选一」的双地图 bug（根因：`MaxFloor=1` 让旧生成退化成 2 节点）。

> 未做（后续可选）：`IMapView` 表现层接口（把 `TreeMapUI` UITK 藏到接口后，便于替换为 uGUI 版地图）；silverua 的去交叉连线（`RemoveCrossConnections`）未移植，长地图可能有少量视觉交叉；`LevelRoomType` 与 `RoomType` 仍双存、在 provider 边界映射。
