...
namespace XianTu
{
    ...
    public class GameConfig : ScriptableObject
    {
        ...
        [Header("═══ 技能掉落 ═══")]
        [Tooltip("敌人死亡时掉落技能的概率。0.03=3%。")]
        [Range(0f, 1f)]
        public float 功法掉落概率 = 0.03f;

        [Tooltip("通关战斗房间后额外掉落技能的概率。0.25=25%。")]
        [Range(0f, 1f)]
        public float 通关功法掉落概率 = 0.25f;

        [Header("═══ 模块掉落（V0.4）═══")]
        [Tooltip("通关战斗房间后掉落模块的概率。1.0=100%（每房必掉）。")]
        [Range(0f, 1f)]
        public float 模块掉落概率 = 0.75f;

        [Tooltip("普通房间模块掉落数量（最少）。")]
        [Range(0, 5)]
        public int 模块掉落数量最少 = 1;

        [Tooltip("普通房间模块掉落数量（最多）。")]
        [Range(1, 5)]
        public int 模块掉落数量最多 = 2;

        [Tooltip("精英房模块掉落数量。")]
        [Range(1, 6)]
        public int 精英房模块掉落数量 = 3;
