...
namespace XianTu
{
    ...
    public class VillageHub : MonoBehaviour
    {
        ...
        private void BuildNpcAndPortal(Action onPortalEntered)
        {
            ...
            riftGo.transform.localPosition = new Vector3(RoomWidth / 2f - 4f, 0f, 0f);
            riftGo.AddComponent<RiftEntrance>().Build();
    ...
    // ============================================================
    //           NPC：Build 管理使（局外 Build 背包查看）
    // ============================================================

    /// <summary>
    /// Build 管理使 NPC（V0.4.1）：玩家走近按 F → 打开 BuildBackpackUI 查看已保存的 Build。
    /// </summary>
    public class BuildManagerNPC : MonoBehaviour, IInteractable
    {
        public Vector3 InteractionWorldPos => transform.position;
        ...
        public void Build()
        {
            ...
            if (bodyRend != null)
            {
                ...
                mat.EnableKeyword("_EMISSION");
                mat.SetColor("_EmissionColor", new Color(0.7f, 0.5f, 0.2f) * 0.5f);
                bodyRend.material = mat;
            }

            _headCard = NpcHeadCard.Attach(transform, new NpcHeadCard.Config
            {
                displayName = "Build 管理使",
                icon = "📦",
                roleSub = "Build 背包",
                hintText = "按 [F] 查看 Build",
                themeColor = new Color(0.85f, 0.65f, 0.35f),
                yOffset = 2.6f,
                showLongRangeMarker = true
            });
        ...
        private void Update()
        {
            if (_headCard != null)
            {
                bool wantHint = IsRoutedActive && !BuildBackpackUI.IsVisible;
                _headCard.SetHintVisible(wantHint);
            }

            if (!IsRoutedActive) return;
            if (BuildBackpackUI.IsVisible) return;

            var kb = UnityEngine.InputSystem.Keyboard.current;
            if (kb != null && kb.fKey.wasPressedThisFrame)
                BuildBackpackUI.Toggle();
        }
    ...
    // ============================================================
    //          大秘境入口：通向局外挑战（需有 Build）
    // ============================================================

    /// <summary>
    /// 大秘境入口（V0.4.1 Phase3 · GDD §11.4.1）：
    /// 玩家走近按 F → 进入大秘境缓冲区。
    /// 前置条件：局外 Build 背包至少有 1 套 Build，否则提示"需先带出 Build"。
    /// </summary>
    public class RiftEntrance : MonoBehaviour, IInteractable
    {
        public Vector3 InteractionWorldPos => transform.position;
        public int InteractionPriority => 5;
        ...
        private static bool HasAnyBuild()
        {
            var bp = SaveSystem.Instance.Data?.buildBackpack;
            return bp != null && bp.Count > 0;
        }

        public void Build()
        {
            // 立柱 + 门帘（红紫色，区别于山门的紫色）
            BuildPillar(new Vector3(-1.8f, 1.5f, 0));
            BuildPillar(new Vector3(1.8f, 1.5f, 0));

            var curtain = GameObject.CreatePrimitive(PrimitiveType.Cube);
            curtain.name = "RiftCurtain";
            curtain.transform.SetParent(transform, false);
            curtain.transform.localPosition = new Vector3(0, 1.5f, 0);
            curtain.transform.localScale = new Vector3(3.2f, 3f, 0.1f);
            var ccol = curtain.GetComponent<Collider>();
            if (ccol != null) Destroy(ccol);
            var crend = curtain.GetComponent<Renderer>();
            if (crend != null)
            {
                ...
                mat.SetInt("_ZWrite", 0);
                mat.renderQueue = 3000;
                mat.color = new Color(0.85f, 0.2f, 0.5f, 0.4f);
                mat.EnableKeyword("_EMISSION");
                mat.SetColor("_EmissionColor", new Color(0.9f, 0.2f, 0.55f) * 2f);
                crend.material = mat;
            }

            var trig = new GameObject("RiftTrigger");
            trig.transform.SetParent(transform, false);
            var sc = trig.AddComponent<SphereCollider>();
            sc.isTrigger = true;
            sc.radius = 2.5f;
            ...
            _headCard = NpcHeadCard.Attach(transform, new NpcHeadCard.Config
            {
                displayName = "大秘境",
                icon = "◈",
                roleSub = "局外挑战 · 装备 Build 验证",
                hintText = "按 [F] 进入大秘境",
                themeColor = new Color(0.9f, 0.3f, 0.6f),
                yOffset = 4.5f,
                showLongRangeMarker = true
            });
        }
        ...
        {
            var pillar = GameObject.CreatePrimitive(PrimitiveType.Cylinder);
            pillar.name = "RiftPillar";
            pillar.transform.SetParent(transform, false);
            pillar.transform.localPosition = localPos;
            pillar.transform.localScale = new Vector3(0.45f, 1.5f, 0.45f);
            var col = pillar.GetComponent<Collider>();
            if (col != null) Destroy(col);
            var rend = pillar.GetComponent<Renderer>();
            if (rend != null)
            {
                var mat = new Material(MaterialHelper.GetLitShader());
                mat.color = new Color(0.4f, 0.12f, 0.3f);
                mat.EnableKeyword("_EMISSION");
                mat.SetColor("_EmissionColor", new Color(0.6f, 0.15f, 0.4f) * 0.5f);
                rend.material = mat;
            }
        }
        ...
        private void Update()
        {
            ...
            bool hasBuild = HasAnyBuild();
            if (_headCard != null)
            {
                _headCard.SetHintVisible(IsRoutedActive);
                _headCard.UpdateHintText(hasBuild ? "按 [F] 进入大秘境" : "需先带出 Build 才能进入");
            }
            ...
            if (kb != null && kb.fKey.wasPressedThisFrame)
            {
                if (!hasBuild)
                {
                    Debug.Log("<color=yellow>[大秘境] 需先完成一次秘境探索带出 Build 才能进入</color>");
                    return;
                }
                if (_headCard != null) _headCard.SetHintVisible(false);
                _triggered = true;
                GameManager.Instance.EnterRift();
            }
        }
