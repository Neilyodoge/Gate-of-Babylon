...
白昼与永夜各固定包含 1 个 Layout 与 1 个 Strength 事件。出生区侧事件节点在昼夜分别承载 1004 / 1007，另一侧节点分别承载 1005 / 1006。
...
1. 摧毁主镜：击败 2 名远程禁卫与 1 名队长后，最后摄政官失去冠光范围技。
2. 偏转镜组：冠光范围技改为锁定 Boss 脚下。
3. 保持完整：冠光范围技继续追踪玩家。
...
1. 修复机构：事件房与王城中庭形成双向快捷通道。
2. 斩断配重：只生成事件房返回中庭的单向通道。
3. 保持封锁：不改变标准路线。

同一事件房 Prefab 通过 EventID 激活四套变体。永夜当前事件可与同节点已提交的白昼实体结果同时显示；永夜死亡后恢复 Flag、完成态、场景终态与升降井入口。
